...
Octant::Octant(uint a_nMaxLevel, uint a_nIdealEntityCount)
{
	...
	*/
	...
	lMinMax.push_back(vector3(-40.0f));
	lMinMax.push_back(vector3(40.0f));
...
bool Octant::IsColliding(uint a_uRBIndex)
{
	...
	vector3 otherMin = m_pEntityMngr->GetRigidBody(a_uRBIndex)->GetMinGlobal();
	vector3 otherMax = m_pEntityMngr->GetRigidBody(a_uRBIndex)->GetMaxGlobal();

	bool colliding = m_v3Min.x <= otherMax.x && m_v3Max.x >= otherMin.x &&
		m_v3Min.y <= otherMax.y && m_v3Max.y >= otherMin.y &&
		m_v3Min.z <= otherMax.z && m_v3Max.z >= otherMin.z;
	...
	return colliding; // for the sake of startup code
}
...
void Octant::Subdivide(void)
{
	...
	float halfWidth = m_fSize / 4;
	centers.push_back(vector3(m_v3Center.x + halfWidth, m_v3Center.y + halfWidth, m_v3Center.z + halfWidth));
	centers.push_back(vector3(m_v3Center.x + halfWidth, m_v3Center.y + halfWidth, m_v3Center.z - halfWidth));
	centers.push_back(vector3(m_v3Center.x - halfWidth, m_v3Center.y + halfWidth, m_v3Center.z + halfWidth));
	centers.push_back(vector3(m_v3Center.x - halfWidth, m_v3Center.y + halfWidth, m_v3Center.z - halfWidth));
	centers.push_back(vector3(m_v3Center.x - halfWidth, m_v3Center.y - halfWidth, m_v3Center.z + halfWidth));
	...
	for (int i = 0; i < 8; i++) {
		m_pChild[i] = new Octant(centers[i], m_fSize / 2);
